(function($) {
	$.fn.slide = function(options) {
		...
		function toLast(move) {
			var objTag = $(settings.slideContainer)[0].tagName; // $('.slideInner div') 鎴� $('.slideInner a')
			switch (move) {
				case 'last':
					el.find(objTag + ':first').css('margin-left', (el.children().length * 100) + "%");
					el.changeSlide(settings, -el.children().length * 100);
					el.find(objTag + ':first').imgMoveSetting('right');
					setTimeout(function() {
						el.css('left', '0%');
						el.find(objTag + ':first').css('margin-left', '0');
						el.find(objTag + ':first').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
				case 'first':
					el.find(objTag + ':last').css('margin-left', -(el.children().length * 100) + "%");
					el.changeSlide(settings, 100);
					el.find(objTag + ':last').imgMoveSetting('left');
					setTimeout(function() {
						el.css('left', -((el.children().length - 1) * 100) + "%");
						el.find(objTag + ':last').css('margin-left', '0');
						el.find(objTag + ':last').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
			}
		...
		$("<ul style='width:" + ($(settings.slideContainer).length * 12 + ($(settings.slideContainer).length - 1) * 10) + "px;margin-left:-" + ($(settings.slideContainer).length * 12 + (($(settings.slideContainer).length - 1) * 5)) / 2 + "px' class='navListBox'>" + navlist + "</ul>").appendTo(el.parent());
